...
class Node:
    ...
    def __init__(self, data):
        self.data = data
        self.next = None ## 아직 다른 노드가 만들어지지 않았으니까 NONE 로 초기화
...
class LinkedList:
    ...
    """
    ## 각각의 노드를 개별적으로 만들고, 그 노드들을 연결해서 리스트를 구성함! 
    ## head 는 리스트의 시작점(첫 번째 노드)을 가리키는 포인터 역할을 함.
    def __init__(self):
        self.head = None

    def append(self, data): ## 이 data 로 노드를 만들어야 됨.
        """
        리스트 끝에 노드 추가
        ...
        ② 이미 노드가 있을 때
              ...
              head ─▶ [1|●]─▶[2|●]─▶[7|None]
        """
        new_node = Node(data) ## 새 노드 만들기 여기서 class 위에 있는 class node 를 불러와서 data 를 넣어줌. 당연히 next 는 None 으로 초기화 됨.
        if self.head is None: ## 해당 linkedLIst 가 비어있으면 head 에 새 노드를 꽂고 끝내기
            self.head = new_node
            return
        cur = self.head ## 이동식 cur 을 설정해줘야됨. 아니면 self.head 가 계속 바뀌어버림. head 는 리스트의 시작점이니까 바뀌면 안됨.
        while cur.next is not None: ## head 부터 시작해서 마지막 노드까지 이동. 마지막 노드는 next 가 None 인 노드임.
            cur = cur.next
        cur.next = new_node        
